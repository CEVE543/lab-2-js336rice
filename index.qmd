...
```{julia}
target_location = "Lubbock, Texas"  # <1>
target_lon = -101.90430            # <2>
target_lat = 33.5845               # <3>
```
...
μ = 1.7    # Location parameter - shifts the distribution left/right
σ = 1.0    # Scale parameter - controls spread (must be > 0)
ξ = 0.05  # Shape parameter - controls tail behavior
...
Station Name: Thorndale 

Thorndale seems to have a weak upward trend, mostly as a result of the most extreme rainfall measurements.  The annual maximum baseline remains the same, but the high values seem to keep getting higher. 


```{julia}
positive_stationid = 684 #Set station id
positive_station = stations[stations.stnid .== positive_stationid, :] #get station row
print(positive_station)

positive_rainfall = @chain rainfall_data begin
	@filter(stnid == !!positive_stationid)  # <1>
end
plot_time_series(positive_station, positive_rainfall)
```
...
Morse seems to have a negative rainfall trend, due in part to a spike in the 1950's.  Still, even disregarding that, there does seem to be a decrease in annual rainfall extremes from the early 1960's until the early 80's, after which things seem to even out. 

```{julia}
positive_stationid = 504 #Set station id
positive_station = stations[stations.stnid .== positive_stationid, :] #get station row
print(positive_station)
...
- Final: $\mu = 1.7$ $\sigma = 1.0$, $\xi = 0.05$
...
Increasing this parameter moves our blue line upwards on the graph, decreasing it moves it down.  


**What happened when you increased/decreased $\sigma$ (scale parameter)?**


Decreasing this parameter flattens out our curve, increasing it makes it curvy again.  
...
Increasing this parameter when it's already above 0 makes the upward curve more extreme.  Setting it to 0 gives a straight line towards the end of the curve.  Decreasing it below 0 gives a line that increases at a slower rate as we move along it. 
...
It follows the station decently well!  Although it has its biggest deviations at the top and bottom portions of the curve. 



**Additional observations:**

The most noticeable trends in rainfall increasing and decreasing seem to be taking place in the most extreme years. 
